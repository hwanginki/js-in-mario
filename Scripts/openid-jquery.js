...
openid = {
    ...
    openPopupWindow : function() {
        $('<div />').addClass('openidblock').appendTo('body');
        var w = window.open('', 'openid_popup', 'width=450,height=500,location=1,status=1,resizable=yes');
        var coords = openid.getCenteredCoords(450, 500);
        w.moveTo(coords.x, coords.y);

        var iv = setInterval(function() {
            if($('.openidblock').length === 0) {
                clearInterval(iv);
            } else if(w.closed) {
                window.handleOpenIdFailure();
                clearInterval(iv);
            }
        }, 200);
    },
	...
	submit : function() {
        ...
        openid.openPopupWindow();
        $('#openid_form').attr('target', 'openid_popup');
		return true;
	},
